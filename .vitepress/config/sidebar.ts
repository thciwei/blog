...
const sync = fg.sync;
//配置分类侧边栏table of contents 按时间或类别
export const sidebar: DefaultTheme.Sidebar = {
  "/tech": getItemsByCategory("posts/tech"),
  "/knowledge/java": getItemsByCategory("posts/knowledge/java"),
  "/projects/mydb": getItemsByCategory("posts/projects/mydb"),
  "/notes/6.5840": getItemsByCategory("posts/notes/6.5840"),
  "/notes/system_architect": getItemsByInnerCategory(
    "posts/notes/system_architect"
  ),
...
function getItemsByCategory(path: string) {
  ...
  if (fs.existsSync(`${path}/index.md`)) {
    ...
    data.showChapterCount !== undefined
      ? (showChapterCount = data.showChapterCount)
      : (showChapterCount = true);
    data.showChapterCountName !== undefined
      ? (showChapterCountName = data.showChapterCountName)
      : (showChapterCountName = "篇");
    data.needRoute !== undefined
      ? (needRoute = data.needRoute)
      : (needRoute = false);
  }
  ...
  groups.push({
    text: `${chapter !== "" ? chapter : path} ${
      showChapterCount && items.length > 0
        ? `(${items.length}${showChapterCountName})`
        : ""
    }`,
...
function getItemsByInnerCategory(path: string) {
  ...
  sync(`${path}/*`, {
    ...
    .sort((a, b) => {
      if (
        fs.existsSync(`${a.path}/index.md`) &&
        fs.existsSync(`${b.path}/index.md`)
      ) {
